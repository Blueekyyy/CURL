...
def render_hair(viewpoint_camera, pc : GaussianModel, pc_hair: GaussianModelHair, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0):
    ...
    conic_precomp = torch.cat([
        #从原始 [405652, 3] 的张量中，选出了 16832 个三维向量
    ...
    #创建零张量。我们将使用它来让 PyTorch 返回二维（屏幕空间）均值的梯度。
    ...
    dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
    sh2rgb = eval_sh(pc_hair.active_sh_degree, shs_view, dir_pp_normalized)  #颜色渲染出以后和diffusion texture去比较？
