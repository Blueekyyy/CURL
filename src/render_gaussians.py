...
@torch.no_grad()
def render_sets(dataset : ModelParams, optimizer: OptimizationParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, scene_suffix : str, pointcloud_path_head=None):
    gaussians = GaussianModel(dataset.sh_degree)
    #import pdb;pdb.set_trace()
    scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, scene_suffix=scene_suffix, pointcloud_path=pointcloud_path_head)
...
if __name__ == "__main__":
    ...
    render_sets(dataset, optimizer.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.scene_suffix, args.pointcloud_path_head)