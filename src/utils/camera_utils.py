...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if hasattr(args, 'load_synthetic_rgba') and args.load_synthetic_rgba:
        ...
        resized_pct_loss = torch.load(cam_info.image_path.replace(f'images_2', f'vgg_features_pct_pth_2').replace(f'png', f'pth'))
        resized_edge_loss = torch.load(cam_info.image_path.replace(f'images_2', f'edge_canny_pth_2').replace(f'png', f'pth'))
    else:
        ...
        #=== Test ===
        #import pdb;pdb.set_trace()
        resized_pct_loss = torch.load(cam_info.image_path.replace(f'images_2', f'vgg_features_pct_pth_1').replace(f'png', f'pth'))
        resized_edge_loss = torch.load(cam_info.image_path.replace(f'images_2', f'edge_canny_pth_1').replace(f'png', f'pth'))
    ...
    #=== Test ===
    #import pdb;pdb.set_trace()
    if hasattr(args, 'load_origin_rgba_geom') and args.load_origin_rgba_geom:
        #resized_image_rgb = PILtoTorch(cam_info.image, resolution)
        resized_image_rgb = PILtoTorch(Image.open(cam_info.image_path.replace(f'images_2', f'images_2_save')), resolution)
        resized_pct_loss = torch.load(cam_info.image_path.replace(f'images_2', f'vgg_features_pct_pth_light_3').replace(f'png', f'pth'))
        resized_edge_loss = torch.load(cam_info.image_path.replace(f'images_2', f'edge_canny_pth_light_3').replace(f'png', f'pth'))
    ...
    #=== Test ===
    gt_pct_loss = resized_pct_loss
    #gt_pct_loss = torch.zeros((1,))
    gt_edge_loss = resized_edge_loss

    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY, width=cam_info.width, height=cam_info.height,
                  image=gt_image, mask_body=gt_mask_body, mask_hair=gt_mask_hair, orient_angle=gt_orient_angle, orient_conf=gt_orient_conf, depth=gt_depth, pct_loss = gt_pct_loss, edge_loss = gt_edge_loss,
