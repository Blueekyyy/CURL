...
class Lie():
    ...
    def se3_to_SE3(self,wu): # [...,3]
        ...
        wx = self.skew_symmetric(w)
        #计算旋转角度模长（范数），是每个旋转向量的角度大小，用来控制后面的 Taylor 展开
        theta = w.norm(dim=-1)[...,None,None]
        #单位矩阵，用于构造 Rodrigues 公式或 Taylor 展开
        I = torch.eye(3,device=w.device,dtype=torch.float32)
        A = self.taylor_A(theta)
        B = self.taylor_B(theta)
        C = self.taylor_C(theta)
        #构造旋转矩阵R=I+A(θ)[ω]+B(θ)[ω]^2
        R = I+A*wx+B*wx@wx
        #构造右乘矩阵（用于变换平移）V=I+B[ω]+C[ω]^2
        V = I+B*wx+C*wx@wx
        #构建 Rt：旋转和平移合并为 3x4 仿射矩阵
