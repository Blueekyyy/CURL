...
class GaussianModel:
    ...
    def restore_ljy(self, model_args, training_args = None):
        (
            self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
        ...
        if training_args is not None:
            self.training_setup_ljy(training_args)
            self.xyz_gradient_accum = xyz_gradient_accum
            self.denom = denom
            #self.optimizer.load_state_dict(opt_dict)
    ...
    def get_features(self):
        ...
        #batch_size = 100000  # 按点数量分批

        #output_list = []
        #for i in range(0, features_dc.shape[0], batch_size):
        #    dc_batch = features_dc[i:i+batch_size]
        #    rest_batch = features_rest[i:i+batch_size]
        #    output_list.append(torch.cat((dc_batch, rest_batch), dim=1))
    ...
    def get_covariance_2d(self, viewpoint_camera, scaling_modifier = 1):
        ...
        #不是拼接，而是堆叠，torch.cat(..., dim=1)	会把维度加长（比如 [405652, 6]）， torch.stack(..., dim=-1)	会增加一个新维度，得到更高阶结构
        #torch.stack把多个向量“排在一起”， torch.cat把多个向量“接在一起”
        J = torch.stack(
            [   
                #[405652],[405652],[405652] -> [405652, 3]
    ...
    def training_setup(self, training_args):
        ...
        #得到了一个可调用函数，它是 helper，并且它“记住”了外层函数的变量 start, end, epochs —— 这就是 闭包（closure） 的作用
        #可以像普通函数一样使用它 lr = self.xyz_scheduler_args(10)
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.position_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)
        
    def training_setup_ljy(self, training_args):
        #import pdb;pdb.set_trace()
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def load_ply(self, path):
        ...
        # === Test ===
        #labels[:, [0]] = inverse_sigmoid(0.5 * torch.ones((xyz.shape[0], 1), dtype=torch.float))
        try:
            labels[:, 0] = np.asarray(plydata.elements[0]["label_0"])
        except:
            #labels[:, 0] = np.zeros(xyz.shape[0])    #没有label_0字段时全0
            labels[:, [0]] = inverse_sigmoid(0.5 * torch.ones((xyz.shape[0], 1), dtype=torch.float))
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        # Extract points that satisfy the gradient condition
        #对于selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              #torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
        #这行进一步收紧筛选条件：
        #torch.max(self.get_scaling, dim=1).values：取每个点三个方向（x/y/z）的最大缩放因子，代表这个点的“最大尺寸”；
        #self.percent_dense * scene_extent：定义一个尺寸上限，越小的高斯点越可能被 densify。
        #于是这一行就是：
        #进一步筛选出尺寸小于一定阈值（即比较小的点），并且之前梯度也足够大，这种点才会被 densify。
    ...
    def densify_and_clone_ljy(self, grads, grad_threshold, scene_extent):
        # Extract points that satisfy the gradient condition
        #对于selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              #torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
        #这行进一步收紧筛选条件：
        ...
        #=== Test ===
        #import pdb;pdb.set_trace()
        selected_pts_mask = torch.where(self.get_label[..., 0] >= 0.5, True, False)
        #selected_pts_mask = torch.where(torch.norm(grads, dim=-1) <= 10000, True, False)
        #selected_pts_mask = torch.logical_and(selected_pts_mask,
        #                                      torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_orient_confs, new_labels, new_scaling, new_rotation)
        del new_xyz, new_features_dc, new_features_rest, new_opacities, new_orient_confs, new_labels, new_scaling, new_rotation
    ...
    def densify_and_prune_ljy(self, max_grad, min_opacity, extent, max_screen_size):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_clone_ljy(grads, max_grad, extent)
        #self.densify_and_split(grads, max_grad, extent)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        #计算 张量的范数
        #input：输入张量，p：范数的类型（L0, L1, L2, ∞，也支持任意实数或 'fro'、'nuc'），dim：在哪个维度上计算范数，keepdim：是否保持维度
        #p=1：L1 范数，绝对值之和
        #p=2：L2 范数（默认）
        #p='fro'：Frobenius 范数（适用于矩阵）
        #p=float('inf')：无穷范数，最大绝对值
        #这段函数的作用是：
        #在每次反向传播后，根据视图空间中点的位置梯度，更新每个点的“梯度累积量”和“计数器”，为后续的 densification（密度提升）操作做准备。
        #self.xyz_gradient_accum[update_filter] += ...：把每个点的梯度模长累加到对应点的梯度累积器中。👉 这个变量是用来跟踪某个点在多个视角下对 loss 的响应强度。
        #self.denom[update_filter] += 1：为每个被更新的点的计数器加 1。这个 denom 后续可以用于计算 平均梯度：
        #这段代码通常出现在一个阶段叫作 densification（加密采样） 的过程中。
        #具体来说：
        #我们想要知道哪些点重要、需要复制或加密；
        #“重要”的定义可以是：这个点在图像平面上的梯度很大（说明影响 loss 很多）；
        #所以作者统计了每个点在多视角中的 x/y 梯度强度总和；
        #然后后续就可以根据这个值，挑出那些贡献大的点来复制（加密），从而提升模型精度。
        #import pdb;pdb.set_trace()
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True)  #viewspace_point_tensor.grad[update_filter,:2]在loss.bacward()后计算梯度，选中update_filter为True的行，点数变少，只取前2列，即x/y图像平面上的维度；self.xyz_gradient_accum：[points_num, 1]
