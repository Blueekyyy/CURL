...
class GaussianModelHair:
    ...
    def restore_ljy(self, model_args, training_args, training_args_hair):
        (
            self._scaling,
            self.active_sh_degree, 
            gen_dict,
    ...
    def filter_points(self, viewpoint_camera):
        #同gaussian_model.py，只是self.bet和self.cov的计算位置不同
        #import pdb;pdb.set_trace()
    ...
    def generate_strands(self, iter, num_strands = -1):
        ...
        if num_strands != -1:
            ...
            #调用OptimizableTexturedStrands.forward
        ...
        #self.num_strands * (self.strand_length - 1)每个高斯椭球
    ...
    def initialize_gaussians_hair(self, iter, num_strands=-1):
        ...
        if 'L_diff' in diffusion_dict.keys():
            self.LDiff = diffusion_dict['L_diff']   #在neural haircut里计算，gaussian haircut没有计算，直接拿结果。
        else:
            self.LDiff = None
    ...
    def create_from_pcd(self, data_path, scale = 1e-3):
        ...
        #{'scale': 1.3511611223220825, 'translation': [-0.2963724136352539, 0.828123927116394, -0.10509262979030609]}
        #放大场景，平移
    ...
    def training_setup(self, training_args, training_args_hair):
        #优化器
        #是OptimizableTexturedStrands里self.texture_decoder = UNet的参数？是头发模型的参数，debug
