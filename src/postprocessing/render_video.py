...
def main(blender_path, input_path, exp_name_1, exp_name_3, strand_length, speed_up, max_frames):
    ...
    for frame in frames:
        ...
        # === Test ===
        #intrinsics, pose = load_K_Rt_from_P(None, cameras["%04d" % frame].transpose(0, 1)[:3, :4].numpy())
        #print("Available frames:", cameras.keys())
        #intrinsics, pose = load_K_Rt_from_P(None, cameras["%05d" % frame].transpose(0, 1)[:3, :4].numpy())
    ...
    #=== Test ===
    #speed_up = 1
    #cameras_interp = np.stack(cameras_interp)[frames[0]:frames[-1]:speed_up]
    ...
    #转换文件格式，没有椭球和折线的处理（在第二步、第三步已处理完）
    ...
    #strands_ply = PlyData.read(f'{input_path}/curves_reconstruction/{exp_name_3}/strands/1000_strands.ply').elements[0].data
    strands_npy = (
        np.stack([
            strands_ply['x'], 
            -strands_ply['z'], 
