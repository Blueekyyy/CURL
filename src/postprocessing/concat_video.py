...
def main(input_path, exp_name_3):
    ...
    for i, img_name in tqdm(enumerate(img_names)):
        ...
        # === Test ===
        #gt = Image.open(f'{input_path}/curves_reconstruction/{exp_name_3}/raw_frames/%04d.png' % (int(img_basename))).convert('RGB')
        #gt = Image.open(f'{input_path}/curves_reconstruction/{exp_name_3}/raw_frames/%05d.jpg' % (int(img_basename))).convert('RGB')
        #gt = Image.open(f'{input_path}/curves_reconstruction/{exp_name_3}/raw_frames/%06d.png' % (int(img_basename))).convert('RGB')
        ...
        #print(f"gt_resized.shape:{gt_resized.shape}   render_blender.shape:{render_blender_cropped_resized.shape}   render_3dgs.shape:{render_3dgs.shape}")
        #target_height = 1080
        target_height = min(gt_resized.height, render_blender_cropped_resized.height, render_3dgs.height)
        gt_resized = resize_to_height(gt_resized, target_height)
        render_blender_cropped_resized = resize_to_height(render_blender_cropped_resized, target_height)
        render_3dgs = resize_to_height(render_3dgs, target_height)
        ...
        frame.save(f'{input_path}/curves_reconstruction/{exp_name_3}/frames/%06d.png' % i)
