...
# === Test ===
#img_names = os.listdir(f'{data_dir}/input{args.postfix}')
#filenames = [f'{data_dir}/input{args.postfix}/{img_name}' for img_name in img_names]
...
for filename in tqdm.tqdm(sorted(filenames)):
    with torch.no_grad():
        ...
        # === Test ===
        #import pdb;pdb.set_trace()
        #mask_hair.save(filename.replace(f'input{args.postfix}', f'masks{args.postfix}/hair').replace(args.image_format, 'png'))
        #mask_face.save(filename.replace(f'input{args.postfix}', f'masks{args.postfix}/face').replace(args.image_format, 'png'))
        #mask_body.save(filename.replace(f'input{args.postfix}', f'masks{args.postfix}/body').replace(args.image_format, 'png'))
