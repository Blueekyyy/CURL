...
#sys.path.append(os.path.abspath('../../ext/Q-Align/q_align'))
...
#model_hyper.load_state_dict((torch.load('../../ext/hyperIQA/pretrained/koniq_pretrained.pkl'))) # 加载预训练权重=加载模型的参数（字典形式）
...
def main(args):
    ...
    if os.path.exists(f'{data_path}/raw'):
        ...
        length = int(vid.get(cv2.CAP_PROP_FRAME_COUNT)) # 获取视频文件的总帧数
        source_fps = int(vid.get(cv2.CAP_PROP_FPS)) # 获取视频的帧率,帧率表示视频播放时每秒显示的帧数。
        if args.target_fps:
            target_fps = args.target_fps
        else:
            target_fps = int(math.ceil(256 / length * source_fps))  # math.ceil向上取整
        ...
        for i in tqdm(range(1, length+1)):
            ...
            #img = np.asarray(torchvision.transforms.Resize(2160)(img))
            img = np.asarray(img)
            cv2.imwrite(f'{data_path}/input/{i:06d}.png', img)

            if not (i % (source_fps // target_fps)):  # //整除
                #idx = np.argmax(np.asarray(scores))
                #j = frames_idx[idx]
                #cv2.imwrite(f'{data_path}/input/{j:06d}.png', frames_rgb[idx].astype('uint8'))
                #import pdb;pdb.set_trace()
                #cv2.imwrite(f'{data_path}/input/{i:06d}.png', frames_rgb[i - 1].astype('uint8'))
